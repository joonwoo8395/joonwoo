		...
		try :
			...
			sql = """
					...
					AND
						...
						OR REP_IP_ADDR_5 = '{}')
					LIMIT 1
					""" .format(emsIp, emsIp, emsIp, emsIp, emsIp, emsIp)
...
#select min(rowid) from tacs.TACS_CORE_EMS_RELATION_CHECK;
			sql	= """
					SELECT 
						RELATION_CHECK_YN
					FROM
...
#				emsEqpId	= '\'%s\'' % emsEqpId
	...
	################### 김준우 09-16 추가 ################################
	def updateDistributeYn(self, hint, workId, emsIp, workStaDate) :
		__LOG__.Trace('hint({}), workId({}), emsIp({}), workStaDate({})'.format(hint, workId, emsIp, workStaDate))
		conn		= None
		try :
			conn 	= M6.Connection(self.IRIS, self.IRIS_ID, self.IRIS_PASS, Database='tacs')
			cursor 	= conn.cursor()

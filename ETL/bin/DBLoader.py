...
class DBLoader():
	def __init__(self, section, cfg):
		...
		# -- 2020 05 21 김준우 추가 HOLA 연동 ---------------
		self.ANIMP_CD		= self.cfg.get(section, "ANIMP_CD")
		self.HOLA_CD		= self.cfg.get(section, "HOLA_CD")

		self.interval		= self.cfg.getint(section, '{}_INTERVAL'.format(self.getHostName()) )
	...
	def getEmsData(self, oneEqpInfoDict) :
		...
		if ( expEmsIp == None or expEmsIp == '' ) or ( expEmsNm == None or expEmsNm == '' ) or ( vendor == None or vendor == '' ) or ( eqpTyp == None or eqpTyp == '' or 'EMS' not in eqpTyp ) or ( tacsEqpId == None or tacsEqpId == '' ) :
...
#			__LOG__.Trace(tacsEqpId)
			...
			if self.jsonValue( oneEqpInfoDict, 'cmdWorkTypCd' ) == '7' and vendor == self.ANIMP_CD :
				__LOG__.Trace('Animp Work')

			# --- 2020 05 21 김준우 추가 HOLA 연동 ---
			elif self.jsonValue( oneEqpInfoDict, 'cmdWorkTypCd' ) == '8' and vendor == self.HOLA_CD :
				__LOG__.Trace('HOLA Work')

			elif self.jsonValue( oneEqpInfoDict, 'cmdWorkTypCd' ) == '4'  :
				eqpEmsNm = self.jsonValue( oneEqpInfoDict, 'enbId' )
				__LOG__.Trace('Access Work')

			elif self.jsonValue( oneEqpInfoDict, 'cmdWorkTypCd' ) == '5' and (self.jsonValue(oneEqpInfoDict, 'svrIp') != None and self.jsonValue(oneEqpInfoDict, 'svrIp') != '') and (tacsEqpId != None and tacsEqpId.strip() != '') :
				...
				## 김준우 추가 2020-06-05 #####################################
				relationCheck = self.IRISDB.selectRelationCheck()

				__LOG__.Trace('EMS - NE Relation Check : %s' % relationCheck)

				if relationCheck == None or relationCheck == 'Y' :
					...
					if eqpEmsNm == None or eqpEmsNm == '' :
						ompIdList = self.IRISDB.selectIdOmpData(tacsEqpId)
				...
				elif relationCheck == 'N' :
					eqpEmsNm    = self.IRISDB.selectIpCommonEqpDataRelationOff( svrIpList )
					__LOG__.Trace('NE EQP_ID : %s' % eqpEmsNm)
				else :
					__LOG__.Trace('RelationCheck only Y or N But RelationCheck : %s' % relationCheck)
			#########################################################################
...
#				expEmsNm, expEmsIp, vendor, eqpTyp	= self.IRISDB.selectNmEmsData( emsNm )
		...
		__LOG__.Trace('배포 ems IP : %s' % oneEqpInfoDict['expEmsIp'])
...
#			oneEqpInfoDict['eqpId']		= self.jsonValue( oneEqpInfoDict, 'enbId' )
		...
		# Animp 의 cmdWorkTypCd = 7 / HOLA 의 cmWork
		# 2020 05 21 김준우 추가 -- HOLA 연동을 위한 추가 -----------------------
		if (self.jsonValue( oneEqpInfoDict, 'cmdWorkTypCd' ) == '7' and self.jsonValue( oneEqpInfoDict, 'vendor' ) == self.ANIMP_CD) or (self.jsonValue( oneEqpInfoDict, 'cmdWorkTypCd' ) == '8' and self.jsonValue( oneEqpInfoDict, 'vendor' ) == self.HOLA_CD) or ( self.jsonValue( oneEqpInfoDict, 'eqpId' ) != '' and self.jsonValue( oneEqpInfoDict, 'eqpId' ) != None ):
			expValidYn = 'Y'
	...
	def run(self):
		...
		while not SHUTDOWN:
			...
			try:
				...
				__LOG__.Trace('strIn = {}'.format(strIn))
				strLine = strIn.strip()

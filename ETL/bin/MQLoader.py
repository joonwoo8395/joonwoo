...
class MQLoader:
	...
	def queueDatCtlFileCreate( self, dataMap, lkngUnitDict ) :
		...
		queueDict	= dict()
		tempMap						= dataMap.copy()

		if 'idx' in tempMap :
			del tempMap['idx']
		if 'expEmsIp' in tempMap :
			del tempMap['expEmsIp']
...
#					continue
			...
			for oneExportInfo in self.repo['exportInfo'] :
				...
				dataMapClone	= dataMap.copy()

				if oneExportInfo['eqpTyp'] == self.NOT_MQ_EMS and oneExportInfo['vendor'] == self.ELG :
					self.exportStdOut( dataMapClone, oneExportInfo )

				else :
					self.exportQueue( dataMapClone, oneExportInfo )
